...
void HeapSort(int* A,int n){
    ...
    // Last node is n-1, it's parent will be last parent ie. ((n-1)-1)/2
    ...
    //then heapifying the rest of tree of size i starting from node 0 i.e selection sort logic
    for(int i=n-1;i>=0;i--){
        swap(A[0],A[i]);
        heapify(A,i,0); //now last element is supposed to be gone, so size is now i not n
    }
}
...
//IN sel sort we linearly traverse to find min to right
//why not use heap property insted
